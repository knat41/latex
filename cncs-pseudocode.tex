...
% \Input / \Output — friendlier aliases for \Require / \Ensure. "Require"
% ("precondition") and "Ensure" ("postcondition") are the formal CS-textbook
% terms, but "input"/"output" is the more familiar framing for a ม.5 student
% meeting pseudocode for the first time. Both names work no matter which
% keyword set is active (English or Thai — \algorithmicrequire/\algorithmicensure
% get redefined by \cncs@setthaikeywords / \cncs@setenglishkeywords above, and
% \Input/\Output just point at \Require/\Ensure, so they follow automatically).
% Used the same way as \Require/\Ensure — bare, not with a brace argument:
%   \Input $n > 0$
